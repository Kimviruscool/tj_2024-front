...
//1. 구조 확인
console.log(영화API) //자료원본
//영화API : 객체{boxOfficeResult:값}

console.log(영화API.boxOfficeResult) //자료 결과 정보 , 주간 박스오피스 전체정보
//영화API.boxOfficeResult : {weeklyBoxOfficeList:값}

console.log(영화API.boxOfficeResult.weeklyBoxOfficeList); //주간 박스오피스의 영화 정보
//영화API.boxOfficeResult.weeklyBoxOfficeList : 배열[객체 , 객체, 객체]

console.log(영화API.boxOfficeResult.weeklyBoxOfficeList[1].rank); //2번째 영화 정보의 순위
console.log(영화API.boxOfficeResult.weeklyBoxOfficeList[1].rank); //2번째 영화 정보의 순위
// [객체[0],객체[1],객체[2],객체[3]]
// 객체[1] : {rank : 값 , movie : 값}
console.log(영화API.boxOfficeResult.weeklyBoxOfficeList[1].movieNm); //2번째 영화 정보의 이름
console.log(영화API.boxOfficeResult.weeklyBoxOfficeList[1].openDt); //영화의 개봉일
console.log(영화API.boxOfficeResult.weeklyBoxOfficeList[1].audiAcc); //누적관객수
console.log(영화API.boxOfficeResult.weeklyBoxOfficeList[1].salesAcc); //누적매출액

//2. HTML 출력 함수 : 어디에 무엇을 출력할지 판단하기 실행조건 : JS가 열렸을때 
movieprint(); //함수 실행 : js가 실행될때 함수 1번 실행 해주기
function movieprint(){ // 함수 정의/만들기
   ...
   let 어디에 = document.querySelector('#tablebody')
    ...
    for(let i = 0; i < 영화목록.length;i++){
        ...
        //무엇을에 반복해서 누적 HTML문자열 누적 대입
        무엇을 += `<tr>
                  <td>${영화.rank}</td>
                  <td>${영화목록[i].movieNm}</td>
                  <td>${영화목록[i].openDt}</td>
                  <td>${영화목록[i].audiAcc}</td>
                  <td>${영화목록[i].salesAcc}</td>
                  </tr>`
        console.log(무엇을);
    }
