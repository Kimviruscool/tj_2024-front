/*
    ...
    5. 배열(객체) : [데이터 , 데이터 , 데이터]
     -여러 데이터를 하나의 데이터로 묶음 형식 , 저장 순서(인덱스)
     <선언> let 변수명 = {}['안녕',123,tur,{}]
     호출 변수명 2. 변수명에인덱스[]

    6. 함수(객체) : function(){}
     -여러 실행문;(코드)를 하나로 묶은 형식, 1.선언적 2.익명 3.화살표
     <선언> function 함수명 (매개변수) {실행문; return;}
     <호출> JS : 함수명(); HTML:<마크업 이벤트 속성="함수명()">

    7. 객체 : {속성명 : 데이터 , 속성명 : 데이터 , 속성명 : 데이터}
     여러 데이터를 속성명 과 데이터를 한쌍으로 이루어진 여러 쌍을 묶은 형식(이름표달아준다), 속성명
    ...
    6.클래스 (미리 구성한 객체 틀/설계)
    let 변수명 = new Date(); 현재 날짜와 시간 객체 반환
    7. DOM객체
    D = document = 문서 = HTML = 현재 JS가 실행된 현재 HTML
    O = Object = 객체
    M = model = 모형/본보기 
    = 문서 객체 모형 => HTML은 객체 언어가 아니다 => JS가 HTML의 모든 요소(<마크업>)를 객체화
